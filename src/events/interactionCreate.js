...
// Corrigido: Removido o async daqui para o Render não dar erro
module.exports = (client) => {
    
    // O async mantém-se aqui, pois é onde os awaits são usados
    client.on("interactionCreate", async (interaction) => {
        const { channel, user, guild, customId: cid, member } = interaction;

        // Ignorar interações fora de servidores
        if (!guild) return;
        ...
        try {
            ...
            if (interaction.isStringSelectMenu() && (cid === "menu_ticket" || cid.startsWith("pagamento_"))) {
                ...
                if (!config.CATEGORY_ID) {
                    return interaction.reply({ content: "❌ Erro: Categoria não configurada no ficheiro config.", flags: [MessageFlags.Ephemeral] });
                }

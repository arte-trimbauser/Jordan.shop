...
function getEmbedColor(embed) {
  ...
  if (!Number.isFinite(number)) return "#5865f2";
...
function renderImage(url, alt = "Imagem", className = "embed-image") {
  ...
  return `<a class="image-link" href="${safeUrl}" target="_blank" rel="noopener noreferrer" title="Abrir imagem em tamanho completo">
...
function renderAdditionalInfo(additionalInfo = {}) {
  const {
    openedBy, openedAt, closedBy, closedAt, claimedBy,
    evaluationSent, evaluation, evaluationComment,
    ticketType, ticketLabel, truckyNome, recrutado, fotoNome, duration,
  } = additionalInfo;
  const hasInfo = openedBy || openedAt || closedBy || closedAt || claimedBy ||
    evaluationSent !== undefined || evaluation !== undefined || evaluationComment ||
    ticketType || ticketLabel || truckyNome || recrutado !== undefined || fotoNome || duration;
  if (!hasInfo) return "";
  return `
...
async function gerarTranscript(channel, ticketId, additionalInfo = {}) {
  try {
    if (!channel) throw new Error("Canal do ticket não foi fornecido.");
    ...
    while (true) {
      ...
      if (!batch?.size) break;
      for (const message of batch.values()) allMessages.set(message.id, message);
    ...
    const sorted = Array.from(allMessages.values()).sort((a, b) => a.createdTimestamp - b.createdTimestamp);
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    :root { --bg: #313338; --bg-secondary: #2b2d31; --bg-tertiary: #1e1f22; --text: #dbdee1; --muted: #949ba4; --white: #f2f3f5; --link: #00a8fc; --mention: #c9cdfb; --mention-bg: rgba(88,101,242,.30); --line: rgba(255,255,255,.06); }
    html { background: var(--bg); scroll-behavior: smooth; }
    body { margin: 0; min-height: 100vh; background: var(--bg); color: var(--text); font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; font-size: 15px; line-height: 1.45; }
    a { color: var(--link); }
    ...
    .server { display: flex; align-items: center; gap: 12px; max-width: 1100px; margin: auto; }
    .server-icon { width: 40px; height: 40px; border-radius: 50%; object-fit: cover; background: #5865f2; flex: 0 0 40px; }
    ...
    .ticket-header { background: var(--bg-secondary); border: 1px solid var(--line); border-radius: 12px; padding: 22px; margin-bottom: 22px; box-shadow: 0 6px 20px rgba(0,0,0,.16); }
    .ticket-header h1 { margin: 0 0 8px; color: var(--white); font-size: 24px; }
    .meta { display: flex; flex-wrap: wrap; gap: 8px 18px; color: var(--muted); font-size: 13px; }
    .meta span { display: inline-flex; align-items: center; gap: 5px; }
    .info-card { background: var(--bg-secondary); border: 1px solid var(--line); border-radius: 10px; padding: 16px; margin-bottom: 22px; }
    .info-title { color: var(--white); font-weight: 700; margin-bottom: 12px; }
    .info-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px; }
    .info-grid > div { background: var(--bg-tertiary); border-radius: 8px; padding: 10px 12px; }
    .info-grid span, .info-grid strong { display: block; }
    .info-grid span { color: var(--muted); font-size: 12px; margin-bottom: 3px; }
    .info-grid strong { color: var(--text); font-weight: 600; overflow-wrap: anywhere; }
    .info-full { grid-column: 1 / -1; }
    .message { display: flex; gap: 16px; padding: 8px 8px 8px 0; border-radius: 6px; scroll-margin-top: 80px; }
    ...
    .bot-tag { background: #5865f2; color: white; border-radius: 3px; padding: 1px 4px; font-size: 10px; font-weight: 700; text-transform: uppercase; }
    ...
    .mention { color: var(--mention); background: var(--mention-bg); border-radius: 3px; padding: 0 2px; font-weight: 500; }
    .message-link { text-decoration: none; overflow-wrap: anywhere; }
    .message-link:hover { text-decoration: underline; }
    code { background: #1e1f22; border: 1px solid rgba(255,255,255,.06); border-radius: 4px; padding: 1px 4px; font-family: Consolas, "Courier New", monospace; color: #c9cdfb; }
    .reply-ref { margin-top: 5px; font-size: 12px; color: var(--muted); }
    .reply-ref a { text-decoration: none; }
    .embed { max-width: 680px; margin-top: 8px; padding: 10px 12px 12px; border-left: 4px solid var(--embed-color); background: #2b2d31; border-radius: 4px; }
    .embed-main { display: flex; gap: 12px; }
    .embed-content { flex: 1; min-width: 0; }
    .embed-title { color: var(--white); font-weight: 700; margin-bottom: 4px; }
    .embed-description { white-space: pre-wrap; overflow-wrap: anywhere; }
    .embed-author { color: var(--white); font-weight: 600; font-size: 13px; display: flex; align-items: center; gap: 6px; margin-bottom: 5px; }
    .embed-author img, .embed-footer img { width: 20px; height: 20px; border-radius: 50%; object-fit: cover; }
    .embed-thumbnail { width: 80px; height: 80px; object-fit: cover; border-radius: 4px; }
    ...
    .embed-fields { display: grid; grid-template-columns: 1fr; gap: 8px; margin-top: 10px; }
    .embed-field.inline { display: inline-block; }
    .embed-field-name { color: var(--white); font-weight: 700; font-size: 13px; }
    .embed-field-value { margin-top: 2px; white-space: pre-wrap; overflow-wrap: anywhere; }
    ...
    .system-divider { display: flex; align-items: center; gap: 10px; color: var(--muted); font-size: 12px; margin: 18px 0; }
    .system-divider::before, .system-divider::after { content: ""; height: 1px; background: var(--line); flex: 1; }
    .footer { text-align: center; color: var(--muted); font-size: 12px; padding-top: 24px; }
...
<body>
  ...
  <main class="wrap">
    ...
    for (const msg of sorted) {
      const day = new Intl.DateTimeFormat("pt-PT", { timeZone: "Europe/Lisbon", dateStyle: "full" }).format(new Date(msg.createdTimestamp));
      if (day !== lastDay) {
      ...
      if (msg.reference?.messageId) {
        ...
        html += `<div class="reply-ref">↪ Resposta a <a href="#m-${escapeHtml(msg.reference.messageId)}">${escapeHtml(replyAuthor)}</a></div>`;
      }
      ...
      if (msg.stickers?.size) {
        for (const sticker of msg.stickers.values()) {
          html += `<div class="attachment">🎨 Sticker: <strong>${escapeHtml(sticker.name || "Sticker")}</strong></div>`;
        }
      }
    ...
    <div class="footer">Fim do transcript • ${sorted.length} mensagens exportadas</div>
  </main>
</body>
</html>`;
...
async function sendTranscript(channel, closedBy = 'Staff') {
  try {
    ...
    const ticketId = channel.name.replace('ticket-', '').slice(0, 20);
    const topic = channel.topic || '';
    const [userId, metodo, produto] = topic.split('|');
    const openedBy = userId ? `<@${userId}>` : 'Desconhecido';
    const openedAt = channel.createdAt ? `<t:${Math.floor(channel.createdAt.getTime() / 1000)}:F>` : 'N/A';
    ...
    // Não relançar para não bloquear o fecho do ticket
  }
}

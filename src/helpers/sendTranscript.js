...
function formatDiscordText(value = "") {
    ...
    text = text.replace(/&lt;(a?):([\w~]+):(\d+)&gt;/g, (_, animated, name, id) => {
        ...
        return `<img class="emoji custom-emoji" src="https://cdn.discordapp.com/emojis/${id}.${ext}?size=48&quality=lossless" alt=":${name}:" loading="lazy">`;
    });
    ...
    const lines = text.split('\n').map(line => /^\s*> /.test(line)
        ? `<blockquote>${formatDiscordText(line.replace(/^\s*> /, ''))}</blockquote>`
        : line);
    text = lines.join('\n');
    ...
    text = text.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_, t, u) =>
        `<a href="${escapeHtml(u)}" target="_blank" rel="noopener noreferrer">${formatDiscordText(t)}</a>`);
    text = text.replace(/(https?:\/\/[^\s<]+)/g, (u) =>
        `<a class="message-link" href="${u}" target="_blank" rel="noopener noreferrer">${u}</a>`);
    text = text.replace(/`([^`\n]+)`/g, "<code>$1</code>");
    text = text.replace(/\*\*([^*\n]+)\*\*/g, "<strong>$1</strong>");
...
function renderEmbed(embed) {
    ...
    const author = embed.author?.name ? `<div class="embed-author">${embed.author.iconURL ? `<img src="${escapeHtml(embed.author.iconURL)}">` : ""}<span>${formatDiscordText(embed.author.name)}</span></div>` : "";
    let fields = "";
    ...
    const footer = embed.footer?.text ? `<div class="embed-footer">${embed.footer.iconURL ? `<img src="${escapeHtml(embed.footer.iconURL)}">` : ""}<span>${formatDiscordText(embed.footer.text)}</span></div>` : "";
...
// ============================================================
// CSS DO TRANSCRIPT (inline)
// ============================================================
const CSS = `
*{box-sizing:border-box}
:root{--bg:#313338;--bg2:#2b2d31;--bg3:#1e1f22;--text:#dbdee1;--muted:#949ba4;--white:#f2f3f5;--link:#00a8fc;--mention:#c9cdfb}
html{background:var(--bg)}
body{margin:0;min-height:100vh;background:var(--bg);color:var(--text);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;font-size:15px;line-height:1.45}
a{color:var(--link)}
...
.ticket-header{background:var(--bg2);border:1px solid rgba(255,255,255,.06);border-radius:12px;padding:22px;margin-bottom:22px}
...
.message{display:flex;gap:16px;padding:8px 8px 8px 0;border-radius:6px}
...
code{background:#1e1f22;border:1px solid rgba(255,255,255,.06);border-radius:4px;padding:1px 4px;color:#c9cdfb}
...
.embed-fields{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:8px;margin-top:10px}
...
@media (max-width:700px){.topbar{padding:12px 14px}.wrap{padding:18px 12px 40px}.ticket-header{padding:16px}.ticket-header h1{font-size:20px}.message{gap:10px}.avatar{width:36px;height:36px;flex-basis:36px}.embed-main{flex-direction:column}}
`;
...
<div class="topbar"><div class="server">
...
<div class="body ${content ? "" : "empty"}">${content || "Sem texto"}</div>`;
        ...
        if (msg.reference?.messageId) {
            ...
            const ra = ref?.member?.displayName || ref?.author?.globalName || ref?.author?.username || "mensagem";
            html += `<div class="reply-ref" style="font-size:12px;color:#949ba4">↪ Resposta a <a href="#m-${escapeHtml(msg.reference.messageId)}">${escapeHtml(ra)}</a></div>`;
        }
...
module.exports = async function sendTranscript(channel, fechadoPor) {
    try {
        ...
        // Anexa o HTML como backup (o ficheiro fica sempre disponível no Discord)
        ...
        // Fallback: se o upload Supabase falhar, usa o URL do anexo do Discord
